...
def app():
    ...
    if st.button('Example'):
        st.info("Modified m1A Sites (Positive Sites)")
        st.subheader("""GGGCCAGGUGCCUGAGAUUGAUGUUCCAUCCUACCUGCCUG""")
        st.subheader("""CUCGCCGCAGGUCUGGCUGGAUGAAGGGCACGGCAUAGGUC""")
        
        st.info("Non-modified m1A Sites (Negative Sites")
        st.subheader("""GGGCCGUUGCUCUGACAUGGACACAGCCAGGACAAGCUGCU""")
        st.subheader("""CGGCCUCGGCCUCACAGCGGACUCUCCACGCCCAGCUAGCU""")
    ...
    def cal_dibase_index(seq):
        dibase_index = {'aa': 1, 'ac': 2, 'au': 3, 'ag': 4, 'ca': 5, 'cu': 6, 'cc': 7, 'cg': 8, 'gg': 9, 'ga': 10,
                        'gc': 11,
    ...
    def input_seq():
        ...
        if st.sidebar.button("Submit"):
            ...
            with open('./Sequence_FVs_for_test.csv', mode='w') as fvFile:
            ...
            dataset = np.genfromtxt("Sequence_FVs_for_test.csv", delimiter=",", dtype=float)
            W = dataset[:, 0:inputSize]
            Y = dataset[:, -1]
            std_scale = StandardScaler().fit(W)
            W = std_scale.transform(W)
            load_model = pickle.load(open('m1A-XGBoost.pkl', 'rb'))
            ...
            for i in range(lno):
                ...
                if output_proba[i] > 0.7:
                    st.info("1-mehtyladenosine Site")
                else:
                    st.info("Non-1-mehtyladenosine Site")
